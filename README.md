...
- **`game/`** (`shapes_of_war`) — the game itself. Bevy + ECS. So far it
  ...
  produces and consumes continuously on the map (crops on their real growth
  cycle, forestry year-round, population growth, prosperity, starvation and
  freezing consequences — all ported from the Python economy).
...
skeleton, M1 worldgen, M2 time + settlement — done).
